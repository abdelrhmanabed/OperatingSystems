...
# Read Allocation.csv
with open(f'Allocation.csv', 'r') as file:
    allocation_data = list(csv.DictReader(file))

# Read Request.csv
with open(f'Request.csv', 'r') as file:
    request_data = list(csv.DictReader(file))

# Read Available.csv
with open(f'Available.csv', 'r') as file:
...
if is_safe_sequence(available, allocation, request):
    ...
    print(f"Deadlocked processes:")
    for a in range (request.shape[0]):
        if not finish[a]:
         print("p"+str(a+1))
